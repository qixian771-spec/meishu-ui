...
**Domain:** WebGL liquid-gradient UI background for a desktop web app (React-based)
**Researched:** 2026-07-30
**Confidence:** HIGH (core stack cross-verified across official changelogs, npm, GitHub, and multiple independent analyses)
...
**The shader itself stays the same in both stacks** — the existing `liquid-demo.html` domain-warped simplex-noise fragment shader is the correct SOTA approach. The stack decision is about the *wrapper* around that shader, not the shader.
...
```bash
# Core
npm install three@0.184.0 @react-three/fiber@9 @react-three/drei@10 react@19 react-dom@19

# Shader tooling
npm install -D vite-plugin-glsl@1.5.5 @types/three
...
Use this when the app is **not** React-based, or when ~150 KB of Three.js is unacceptable for a single background shader.
...
npm install -D vite-plugin-glsl@1.5.5
```

**OGL fullscreen-shader pattern** (direct port of the demo):
```js
import { Renderer, Program, Mesh, Geometry } from 'ogl';
import fragment from './liquid.frag';   // via vite-plugin-glsl
import vertex from './fullscr.vert';
...
const program = new Program(gl, {
  vertex, fragment,
  uniforms: { uTime: { value: 0 }, uRes: { value: [0,0] } }
});
...
**What OGL gives up vs Three.js+R3F:** no React lifecycle integration (you manage canvas mount/unmount/cleanup manually), no drei helpers, no post-processing pipeline, smaller community for troubleshooting.

---

## GLSL Shader Approach — Recommendation

The existing demo uses **domain-warped fbm (fractal Brownian motion) of 3D simplex noise**. This is the correct, SOTA approach. **Keep it.**
...
**Shader port checklist** (demo → production component):
1. Extract the `FRAG` string into `src/shaders/liquid.frag` (via `vite-plugin-glsl`)
2. Extract the `VERT` (fullscreen triangle) into `src/shaders/fullscr.vert`
3. Convert `precision highp float` — keep it; `mediump` causes banding artifacts in smooth gradients on some Android GPUs
4. Keep the `snoise` + `fbm` functions as-is (Ashima/MIT, battle-tested)
5. Keep the 5-color palette as `uniform vec3 uColors[5]` instead of hardcoded — enables theme switching
6. Add `uniform float uIntensity` (currently hardcoded `0.9`) — enables reduced-motion fade
7. The demo already caps DPR at 2 — preserve this in the component
...
The project uses **Ardot** (.ardot static canvas) for design, not Figma. This changes the handoff strategy:
...
**Recommendation:** Stay on Ardot for design reference. Build the React UI by hand (it's 4 screens of glassmorphism cards — not worth a tool migration). The shader is already code. No design-to-code automation tool adds value for this specific project.
...
**If the app is React 19-based (most likely for a 2026 desktop web app):**
...
**If the app is framework-light / vanilla JS:**
...
**If bundle size is absolutely critical (< 20 KB total):**
...
---
*Stack research for: WebGL liquid-gradient UI background (灵犀 Nexus)*
*Researched: 2026-07-30*
